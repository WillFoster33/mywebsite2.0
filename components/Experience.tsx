		...
		<div className="space-y-4">
			{config.experience?.map((item) => {
				...
				const isBigger =
					...
					item.company === 'Quanta (Startup)' ||
					item.company === 'Big Bear Adventures' ||
					item.company === 'Ann Arbor Golf and Outing'
				const isSmaller = item.company === 'ArcTwo' || item.company === 'Scale AI/Outlier AI'
				const sizeClass = isBigger ? 'h-16' : isSmaller ? 'h-10' : item.company === 'Lawrence AI' ? 'h-12' : 'h-12'
				return (
